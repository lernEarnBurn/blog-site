...
export function CreateBlogPage(){
  ...
  const {createBlog, createLoading} = useCreateBlog(titleValue, contentValue, navigate)
  ...
  return (
      ...
      > 
        <div className="flex gap-2">
          <CreateBtnBar backFunc={handleGoBack} createLoading={createLoading} createBlog={() => { if(titleValue !== '' && contentValue !== ''){createBlog()}}}></CreateBtnBar>
          <div className="flex flex-col  rounded-lg dark:bg-opacity-90 py-2 px-10 border-2 light:border-black shadow-sm w-[35vw] h-[87vh] overflow-hidden">      
            ...
            <textarea spellCheck="false" onChange={handleContentChange} value={contentValue} rows={21} maxLength={1000}  className="w-[30vw] mt-2 mx-auto ghost-input"/>
          </div>
        </div>
      </motion.div>
  )
...
const useCreateBlog = (title: string, content: string, navigate: NavigateFunction) => {
  ...
  const createBlog = async() => {
    ...
    try{ 
      ...
      const response = await axios.post(`http://localhost:3000/posts`, data, config)
      createBlogLocally()
